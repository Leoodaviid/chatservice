...
import (
	...
	"github.com/leoodaviid/chatservice/internal/domain/entity"
	"github.com/leoodaviid/chatservice/internal/domain/gateway"
...
func (uc *ChatCompletionUseCase) Execute(ctx context.Context, input ChatCompletionInputDTO) (*ChatCompletionOutputDTO, error ){
	...
	if err != nil {
		if err.Error() == "chat not found"{
			...
			return nil, errors.New("error fetching exiting chat " + err.Error())
		}
	}
	...
	if err != nil{
		return nil, errors.New("error creating assistant menssage: " + err.Error())
	}
...
func createNewChat(input ChatCompletionInputDTO) (*entity.Chat, error){
	...
	chatConfig := &entity.ChatConfig{
		...
		FrequencePenalty: input.Config.FrequencyPenalty,
		Model: model,
	}
	initialMessage, err := entity.NewMessage("system", input.Config.InitialSystemMessage, model)
	if err != nil{
		return nil, errors.New("error creatinmg initial message: " + err.Error())
	}
